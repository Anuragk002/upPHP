...
if (isset($_POST['form1'])) {
	...
	if (empty($_POST['rating'])) {
		$valid = 0;
		$error_message .= "You must have to select a product rating<br>";
	}
	...
	if ($valid == 1) {
		...
		// Updating The Rating->
		$statement = $pdo->prepare("DELETE FROM tbl_rating WHERE p_id=? AND cust_id=?");
		$statement->execute(array($_REQUEST['id'],-2));
		$statement = $pdo->prepare("INSERT INTO tbl_rating (p_id,cust_id,rating) VALUES (?,?,?)");
		$statement->execute(array($_REQUEST['id'],-2,$_POST['rating']));
                ...
                <div class="box box-info">
                    <div class="box-body">
                        ...
                        <div class="form-group">
                                            ...
                                            <td>
                                                <div class="col-sm-4">
                                                    <input type="text" placeholder="Package name"
                                                        value="<?php echo $row['pkg_name'] ?>" required
                                                        class="form-control" name="pkg[<?php echo $key?>][name]"
                                            ...
                                            <td>
                                                <div class="col-sm-4">
                                                    <input type="text" placeholder="price"
                                                        value="<?php echo $row['pkg_price'] ?>" required
                                                        class="form-control" name="pkg[<?php echo $key?>][price]"
                                                        style="margin-bottom:5px; width:50px;">
                                                </div>
                                            </td>
                            ...
                            <div class="col-sm-2">
                                <input type="button" id="btnAddNewPkg" value="Add Package"
                        ...
                        <!-- rating start-->
                        <div class="form-group">
                            <label for="" class="col-sm-3 control-label">Product Rating <span>*</span></label>
                            <div class="col-sm-4">
                                <select name="rating" class="form-control select2" required>
                                    ...
                                    <option value="<?php echo $i; ?>"
                                        <?php if($rating_flag==1){if($rating==$i){echo "selected";}}?>>
                                        <?php echo $i; ?></option>
                                    <?php }?>
                                </select>
                            </div>
